...
ll=[] #
code_pattern = r'```([\w\W]+?)```'  #to find code in repply
go=False # to check if weak up call is done
c1=False #to allow first question after weak up call
# Initialize OpenAI API
#openai.api_key = "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"
openai.api_key = "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"
...
def generate_response(prompt):
    ...
    raw_reply = chat.choices[0].message.content
    mss.append({"role": "assistant", "content": raw_reply})
    reply=code_sep(raw_reply)##to serperate out code from reply
    

    return reply
def code_sep(ass_reply):#def to seperate code from text
    ll=[]
    reply=""
    matches = re.findall(code_pattern, ass_reply)
    ll=ass_reply.split(matches)
    for rep in ll:
        reply=reply+rep
